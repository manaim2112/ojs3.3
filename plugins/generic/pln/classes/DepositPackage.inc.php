...
class DepositPackage {
	...
	public function generateAtomDocument() {
		...
		$dateModified = $this->_deposit->getDateModified();
		$entry->appendChild($this->_generateElement($atom, 'updated', $dateModified ? date("Y-m-d H:i:s", strtotime($dateModified)) : date("Y-m-d H:i:s")));
		...
		$pkpDetails->setAttribute('pubdate', $objectPublicationDate ? date('Y-m-d', (is_int($objectPublicationDate) ? $objectPublicationDate : strtotime($objectPublicationDate))) : '');
	...
	public function transferDeposit() {
		...
		$status = $result['status'] ? intdiv((int) $result['status'], 100) : 0;
	...
	public function updateDepositStatus() {
		...
		$statusCode = $result['status'] ? (int) $result['status'] : 0;
