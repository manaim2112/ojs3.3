...
class OpenIDPlugin extends GenericPlugin
{
	...
	public static function encryptOrDecrypt(OpenIDPlugin $plugin, int $contextId, ?string $string, bool $encrypt = true): ?string
	{
		...
		if ($encrypt) {
			...
			$ciphertext = openssl_encrypt($string, $alg, $pwd, OPENSSL_RAW_DATA, $iv);
			$hmac = hash_hmac('sha256', $ciphertext, $pwd, true);
			return base64_encode('v2:' . $iv . $hmac . $ciphertext);
		}
		...
		// New format (v2): random IV + HMAC + ciphertext
		if (substr($decoded, 0, 3) === 'v2:') {
			$payload = substr($decoded, 3);
			$iv = substr($payload, 0, 16);
			$hmac = substr($payload, 16, 32);
			$ciphertext = substr($payload, 48);
			$expectedHmac = hash_hmac('sha256', $ciphertext, $pwd, true);
			if (!hash_equals($expectedHmac, $hmac)) {
				error_log('OpenIDPlugin: HMAC verification failed — data may be tampered');
				return null;
			}
			$result = openssl_decrypt($ciphertext, $alg, $pwd, OPENSSL_RAW_DATA, $iv);
			return $result !== false ? $result : null;
		}

		// Legacy format (v1): IV derived from key, no auth
		$oldIv = substr($pwd, 0, 16);
		return openssl_decrypt($string, $alg, $pwd, 0, $oldIv);
	}
